# Share Templates Batch 2 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add 6 new share templates (Field Report, Impact, Special Report, Alert, Update, Highlight) matching user-supplied reference designs, switch share-card meta text from relative time to an absolute post date everywhere it's used, and fix a bug where `ShareTemplate.paletteColor` was never actually reaching the renderer.

**Architecture:** All new drawing logic lives in `ShareCardRenderer` ([share_card_renderer.dart](../../../lib/features/news/data/share/share_card_renderer.dart)), following the existing pattern: one `TitlePosition` enum value + one static `_draw*` method per layout, dispatched from `ShareCardRenderer.paint`'s switch. Two new shared helpers (`_drawPill`, `_drawCircularLogo`) avoid duplicating pill-badge and circular-logo-clip code across the growing method count. `ShareTemplate` entries in `shareTemplates` are the only thing the UI (carousel/grid in `share_preview_screen.dart`) needs to pick up new templates — no screen changes needed beyond the two noted in Task 1.

**Tech Stack:** Flutter (Dart), `intl` (already a direct pubspec dependency, used here for `DateFormat`).

## Global Constraints

- Every task must end with `flutter analyze` clean (`No issues found!`) and `flutter test` passing, run on the touched files / whole suite respectively.
- `relativeTime` in `date_format.dart` stays unchanged and stays in use outside share cards (dashboard, article list) — only the two share-card call sites switch to the new `postDate`.
- No automated tests are added beyond what the existing `share_card_test.dart` loop already exercises for free (it iterates `shareTemplates`, so new entries are covered automatically).

---

### Task 1: Foundation + Ribbon pill variant + Field Report

Adds: `postDate` formatter (global meta text change), the `paletteColor` wiring bug fix, `BannerShape` enum + config/template fields, the two shared draw helpers, the ribbon's pill banner shape, and the revived/enhanced `panel` layout (Field Report). Produces 3 new selectable templates: Field Report, Special Report, Alert.

**Files:**
- Modify: `lib/core/utils/date_format.dart`
- Modify: `lib/features/news/data/share/share_card_renderer.dart`
- Modify: `lib/features/news/data/datasources/share_data_source.dart`
- Modify: `lib/features/news/presentation/screens/share_preview_screen.dart`

**Interfaces:**
- Produces: `postDate(DateTime time) -> String`; `BannerShape` enum (`notch`, `pill`); `ShareCardConfig.bannerShape` and `ShareTemplate.bannerShape` fields (both default `BannerShape.notch`); `ShareCardRenderer._drawPill(canvas, label, color, {required Offset anchor, required bool centered}) -> Rect`; `ShareCardRenderer._drawCircularLogo(canvas, center, radius, logo, {required Color fallbackColor})`. These four are consumed by Task 2 and Task 3.

- [ ] **Step 1: Add `postDate` to `date_format.dart`**

Replace the entire contents of `lib/core/utils/date_format.dart` with:
...
- [ ] **Step 2: Switch the two share-card meta call sites to `postDate`**

In `lib/features/news/data/datasources/share_data_source.dart`, find:
...
In `lib/features/news/presentation/screens/share_preview_screen.dart`, find:
...
- [ ] **Step 3: Replace `share_card_renderer.dart` with the foundation + Field Report version**

Replace the entire contents of `lib/features/news/data/share/share_card_renderer.dart` with:
...
Note: `TitlePosition.boxedHeadline` and `TitlePosition.updatePill` cases are `break;` stubs (Task 2 fills them in) and `TitlePosition.highlight` is a `break;` stub (Task 3 fills it in) -- they exist in the enum now so `bannerShape`/badge-visibility wiring in Step 4 below can reference them, but nothing selects those `TitlePosition`s yet since no `ShareTemplate` uses them until their tasks land. This keeps the switch exhaustive (no analyzer warnings) at every step.

- [ ] **Step 4: Wire the bug fix and new config field into the screen**
...
Run: `flutter test`
Expected: All tests pass (the existing `share_card_test.dart` loop now also exercises Field Report, Special Report, and Alert automatically since they're in `shareTemplates`).

- [ ] **Step 6: Commit**

```bash
git add lib/core/utils/date_format.dart lib/features/news/data/share/share_card_renderer.dart lib/features/news/data/datasources/share_data_source.dart lib/features/news/presentation/screens/share_preview_screen.dart
git commit -m "$(cat <<'EOF'
Add Field Report, Special Report, and Alert share templates

Introduce a shared pill-badge helper and a circular-logo helper,
give the ribbon banner a plain-pill shape option (used by Special
Report and Alert), and revive/enhance the unused panel layout with a
description, content-driven height, and an over-photo badge (Field
Report). Also fix ShareTemplate.paletteColor never reaching the
renderer, and switch share-card meta text from relative time to an
absolute post date.
EOF
)"
```
...
### Task 2: Impact and Update

Adds the `boxedHeadline` and `updatePill` layouts and their templates.

**Files:**
- Modify: `lib/features/news/data/share/share_card_renderer.dart`

**Interfaces:**
- Consumes: `_drawPill`, `_drawBottomScrim`, `_breakingRed`, `_white`, `_nearBlack` (all from Task 1).
- Produces: nothing new consumed elsewhere -- `boxedHeadline`/`updatePill` are only referenced via their `ShareTemplate` entries, same as every other layout.

- [ ] **Step 1: Fill in the `boxedHeadline` and `updatePill` switch cases**

In `lib/features/news/data/share/share_card_renderer.dart`, find:
...
- [ ] **Step 2: Add the two new draw methods**

In `lib/features/news/data/share/share_card_renderer.dart`, find the end of `_drawPanel` (the closing `}` right before the `/// Full dark overlay with a centered category pill...` doc comment for `_drawSpotlight`), and insert these two new methods between them:
...
- [ ] **Step 3: Add the two new `ShareTemplate` entries**
...
- [ ] **Step 4: Show the badge toggle for these two templates as well**
...
Run: `flutter test`
Expected: All tests pass (Impact and Update now exercised automatically via the `shareTemplates` loop).

- [ ] **Step 6: Commit**

```bash
git add lib/features/news/data/share/share_card_renderer.dart lib/features/news/presentation/screens/share_preview_screen.dart
git commit -m "$(cat <<'EOF'
Add Impact and Update share templates

Impact: a solid-color box floating over the lower photo, sized to the
headline, with a badge above and description/meta below. Update: a
centered pill badge in the upper-middle of the photo with the
headline and meta stacked below it.
EOF
)"
```
...
Adds the `highlight` layout: full-bleed photo, marker-highlighted headline (colored bar behind each wrapped line via `computeLineMetrics()`), description, meta, and a large centered logo at the bottom instead of the usual small corner mark.

**Files:**
- Modify: `lib/features/news/data/share/share_card_renderer.dart`

**Interfaces:**
- Consumes: `_drawCircularLogo`, `_white`, `_breakingRed` (all from Task 1).

- [ ] **Step 1: Fill in the `highlight` switch case and skip the corner brand mark for it**
...
- [ ] **Step 2: Add the `_drawHighlight` method**

Find the end of `_drawUpdatePill` (the closing `}` right before the `/// Full dark overlay with a centered category pill, headline, and meta --` doc comment for `_drawSpotlight`), and insert this new method right after `_drawUpdatePill`:
...
- [ ] **Step 3: Add the `Highlight` `ShareTemplate` entry**
...
Run: `flutter test`
Expected: All tests pass (Highlight now exercised automatically via the `shareTemplates` loop, including its `computeLineMetrics()` call with the test article's real multi-line title).

- [ ] **Step 5: Manual check in the running app**

This is the one layout worth a visual glance if you can run the app (`flutter run`) -- open the share screen, select "Highlight", and confirm: the headline's highlight bars land behind each wrapped line (not just the first), the description/meta read clearly below it, and the logo renders large and centered at the bottom with no duplicate small logo in the top-right corner.

- [ ] **Step 6: Commit**

```bash
git add lib/features/news/data/share/share_card_renderer.dart
git commit -m "$(cat <<'EOF'
Add Highlight share template

Full-bleed photo with a marker-highlighted headline (colored bar
behind each wrapped line), description, meta, and the logo shown
large and centered at the bottom instead of the usual small corner
mark.
EOF
)"
```
